...
# unless the --multiqc-off flag is provided.
# Optionally, the --multiqc-dir flag allows specifying a different directory for MultiQC to search.
...
help(){
    ...
    echo "By default, MultiQC will search the FastQC output directory."
    echo "This can be overridden by specifying --multiqc-dir"
    echo ""
    ...
    echo ""
    echo "Usage: $0 --fastq <fastq_directory> --output <output_directory> [--multiqc-dir <multiqc_directory>] [--multiqc-off]"
    echo ""
    echo "Options:"
    ...
    echo "  --multiqc-dir      (Optional) Path to directory to search for FastQC reports for MultiQC."
    echo "                     Defaults to the output directory if not specified."
...
multiqc_dir=""       # optional; if not provided, defaults to output_dir
...
# If --multiqc-dir is not specified, default to the output directory.
if [ -z "$multiqc_dir" ]; then
    multiqc_dir="$output_dir"
    echo "MultiQC directory not specified; defaulting to output_dir: $multiqc_dir"
fi
...
# Create the MultiQC directory if it doesn't exist.
if [ ! -d "$multiqc_dir" ]; then
    echo "MultiQC directory '$multiqc_dir' does not exist."
    echo "Creating MultiQC directory..."
    mkdir -p "$multiqc_dir"
fi
